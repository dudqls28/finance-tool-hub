...
const WHY_USE_ITEMS_KO = [
  { title: '무료로 사용', description: '회원가입 없이 모든 계산기를 무료로 이용할 수 있습니다.' },
  { title: '즉시 결과', description: '입력만 하면 바로 결과가 나와 빠르게 계획을 세울 수 있습니다.' },
  { title: '모바일 최적화', description: '휴대폰에서도 편하게 사용할 수 있도록 반응형으로 구성했습니다.' },
]
const WHY_USE_ITEMS_EN = [
  { title: 'Free to use', description: 'All calculators are free with no sign-up required.' },
  { title: 'Instant results', description: 'Get results immediately and plan faster.' },
  { title: 'Mobile-friendly', description: 'Responsive design for use on any device.' },
]
...
const HOME_FAQ_EN = [
  { q: 'Are the calculators free?', a: 'Yes. All calculators are free and require no sign-up.' },
  { q: 'Is my data saved?', a: 'No. Inputs stay in your browser and are not sent to any server.' },
  { q: 'Why do I see ads?', a: 'We use Google AdSense to support the service.' },
  { q: 'Can I trust the results?', a: 'For reference only. Consult professionals for real financial decisions.' },
]
...
function groupByCategory(tools: ToolConfig[], locale: 'ko' | 'en') {
  const labels = locale === 'en' ? CATEGORY_LABELS_EN : CATEGORY_LABELS_KO
  ...
  return order.filter((c) => map.has(c)).map((c) => ({ category: c, label: labels[c], tools: map.get(c)! }))
}
...
export function Home() {
  ...
  const whyUseItems = locale === 'en' ? WHY_USE_ITEMS_EN : WHY_USE_ITEMS_KO
  const homeFaq = locale === 'en' ? HOME_FAQ_EN : HOME_FAQ_KO
  const desc = locale === 'en' ? HOME_DESCRIPTION_EN : HOME_DESCRIPTION_KO

  return (
    <div className="flex min-h-screen flex-col">
      <Seo title={HOME_TITLE} description={desc} />
      <Header />
      <main className="flex-1">
        ...
        <section className="border-b border-slate-200 bg-white py-12 sm:py-16">
          <div className="mx-auto max-w-6xl px-4 text-center sm:px-6">
            <h1 className="text-3xl font-bold tracking-tight text-text sm:text-4xl">
              {homeSectionTitles.hero[locale]}
            </h1>
            <p className="mx-auto mt-4 max-w-2xl text-lg text-slate-600">
              {homeSectionTitles.heroSub[locale]}
            </p>
          </div>
        </section>
        ...
        <section className="py-12 sm:py-16">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            <h2 className="text-xl font-semibold text-text">{homeSectionTitles.popular[locale]}</h2>
        ...
        <section className="border-t border-slate-200 bg-slate-50/50 py-12">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            <h2 className="text-xl font-semibold text-text">{homeSectionTitles.categories[locale]}</h2>
        ...
        <section className="border-t border-slate-200 bg-white py-12">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            <h2 className="text-xl font-semibold text-text">{homeSectionTitles.whyUse[locale]}</h2>
        ...
        <section className="border-t border-slate-200 bg-slate-50/50 py-12">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            <h2 className="text-xl font-semibold text-text">{homeSectionTitles.featured[locale]}</h2>
        ...
        <section className="border-t border-slate-200 bg-white py-12">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            <h2 className="text-xl font-semibold text-text">{homeSectionTitles.faq[locale]}</h2>
